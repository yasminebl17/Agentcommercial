    ...
    if (in_array($file_ext, $allowed_ext)) {
        ...
        foreach ($data as $row) {
            if ($count > 0) {
                ...
                $checkQuery = "SELECT * FROM table_cm_4g_lte_realisation WHERE annee = '$annee' AND mois = '$mois'";
                $checkResult = mysqli_query($conn, $checkQuery);

                if (mysqli_num_rows($checkResult) > 0) {
                    // Si l'entrée existe déjà, ajoutez un message d'erreur à la liste
                    $errors[] = "L'entrée avec l'année $annee et le mois $mois table_cm_4g_lte_realisation.";
                }  else {
                    // Insérez la ligne si elle n'existe pas déjà
                    $insertQuery = "INSERT INTO table_cm_4g_lte_realisation  (
